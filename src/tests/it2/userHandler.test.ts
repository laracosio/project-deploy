...
import { authLoginRequest, authUserDetailsRequest, clearRequest, authRegisterRequest, userUpdateDetailsRequest, userUpdatePasswordRequest } from './serverTestHelperIt2';
...
describe('PUT /v1/admin/user/details - Error Cases', () => {
  test('Email is currently used by another user (excluding the current user)', () => {
    ...
    const response = userUpdateDetailsRequest(userData.token, person2.email, person1.nameFirst, person1.nameLast);
  ...
  test('Email does not satisfy: (validator.isEmail)', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, 'h.simpson@@springfield.com', person1.nameFirst, person1.nameLast);
    expect(response.statusCode).toStrictEqual(400);
  ...
  test('NameFirst contains characters other than lowercase, uppercase, spaces, hyphens, or apostrophes - numbers', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, 'St3v3', person1.nameLast);
    expect(response.statusCode).toStrictEqual(400);
  ...
  test('NameFirst contains characters other than lowercase, uppercase, spaces, hyphens, or apostrophes - special', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, 'B@rry', person1.nameLast);
    expect(response.statusCode).toStrictEqual(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ error: 'Name must only contain lowercase letters, uppercase letters, spaces, hyphens, or apostrophes' });
  });
  test('NameFirst is less than 2 characters or more than 20 characters - less', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, 'a', person1.nameLast);
    expect(response.statusCode).toStrictEqual(400);
  ...
  test('NameFirst is less than 2 characters or more than 20 characters - more', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, 'Thisnameismorethan20ch', person1.nameLast);
    expect(response.statusCode).toStrictEqual(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ error: 'Name must be between 2 and 20 characters' });
  });
  test('NameLast contains characters other than lowercase, uppercase, spaces, hyphens, or apostrophes - numbers', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, person1.nameFirst, 'St3v3');
    expect(response.statusCode).toStrictEqual(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ error: 'Name must only contain lowercase letters, uppercase letters, spaces, hyphens, or apostrophes' });
  });
  test('NameLast contains characters other than lowercase, uppercase, spaces, hyphens, or apostrophes - special', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, person1.nameFirst, 'B@rry');
    expect(response.statusCode).toStrictEqual(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ error: 'Name must only contain lowercase letters, uppercase letters, spaces, hyphens, or apostrophes' });
  });
  test('NameLast is less than 2 characters or more than 20 characters - less', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, person1.nameFirst, 'a');
    expect(response.statusCode).toStrictEqual(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ error: 'Name must be between 2 and 20 characters' });
  });
  test('NameLast is less than 2 characters or more than 20 characters - more', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, person1.nameFirst, 'Thisnameismorethan20ch');
    expect(response.statusCode).toStrictEqual(400);
    expect(JSON.parse(response.body.toString())).toStrictEqual({ error: 'Name must be between 2 and 20 characters' });
  });
  test('Invalid token', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token + 1, person1.email, person1.nameFirst, person1.nameLast);
...
describe('PUT /v1/admin/user/details - Success Cases', () => {
  ...
  test('nameFirst updated', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, 'Homes', person1.nameLast);
    expect(response.statusCode).toStrictEqual(200);
    expect(JSON.parse(response.body.toString())).toStrictEqual({});
  });
  test('nameLast updated', () => {
    const user = authRegisterRequest(person1.email, person1.password, person1.nameFirst, person1.nameLast);
    const userData = JSON.parse(user.body.toString());
    const response = userUpdateDetailsRequest(userData.token, person1.email, person1.nameFirst, 'Simps');
    expect(response.statusCode).toStrictEqual(200);
    expect(JSON.parse(response.body.toString())).toStrictEqual({});
  });
});
